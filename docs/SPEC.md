...
- **The gate (reused, not new):** turning the parent view ON requires `parent_lock_flows.authenticateParent` — Face ID (if enabled) → 4-digit PIN → or the **3-second hold** fallback when no parent lock is set. This is the **same lock** as the kid-mode toggle in `main_menu`. No new auth mechanism.
