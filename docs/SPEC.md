...
- **Data (WEB-only — kid compliance):** each kid story bundles `scripture_{id}_web.txt`, generated from its `bibleSourceRef` by `scripts/backfill_kid_scripture.py`. For broad citations where the story draws on a portion (e.g. 1 Kings 6–8, mostly building measurements), the bundled text is the **key passages**; the displayed `bibleSourceRef` citation is unchanged. Each kid story also carries `scriptureKeyVerse` (`{ ref, text }`) — one central, hand-curated verse. Legacy Traditional stories flagged `kidFriendly` (e.g. 1004–1095) carry the same `scriptureKeyVerse` (backfilled by `scripts/backfill_kidfriendly_keyverse.py`) over their existing `scripture_{id}_web.txt`. The key verse is an **enrichment, not the gate** — it may be absent without weakening the gate.
- **Default view (no gate — "simple kid-friendly scripture"):** the Scripture panel / Read Scripture page shows the reference (*"This story comes from {bibleSourceRef}"*) and, when present, the single `scriptureKeyVerse`. **When the key verse is missing, the view falls back to the reference only — never the full passage.** This is the only scripture a child sees ungated.
...
- **Gating rule (fail-closed, LOCKED):** the parent gate keys off the parable's **`kidFriendly` flag ALONE**, never the presence of a key verse. Any `kidFriendly` story opens in the kid-simple view first and keeps the full passage behind the parent unlock; a missing `scriptureKeyVerse` degrades to reference-only — it can never leak scripture.
- **Kid-mode detection:** in kid mode the served story variants are `kidFriendly: true` (gated). Adult / Traditional variants (`kidFriendly: false`) are ungated — full scripture, no gate, exactly as before.
