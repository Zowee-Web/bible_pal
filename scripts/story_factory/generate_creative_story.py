...
All story/reflection prose is generated by a local Ollama model.
Model selection is handled by the Universal Model Router (server/model_router/).
Falls back to gemma:7b if the router is unavailable.
This script generates NO prose itself.

Engine assignment: Creative = Local Ollama ONLY.
...
MODEL = "gemma:7b"  # default; overridden by router at runtime
TEMPERATURE = 0.7
STOP_TOKEN = "<END>"
...
def _resolve_creative_model(root: pathlib.Path) -> str:
    """Resolve creative story model via Universal Model Router.

    Falls back to gemma:7b if the router is unavailable.
    """
    cli_path = root / "server" / "model_router" / "cli.py"
    if not cli_path.exists():
        return "gemma:7b"
    try:
        result = subprocess.run(
            [sys.executable, "-m", "server.model_router.cli", "resolve", "creative_story"],
        ...
        if result.returncode == 0:
            ...
            if model:
                ...
                label = f"(fallback depth {data.get('fallback_depth', '?')})" if is_fb else "(primary)"
                print(f"  Router selected: {model} {label}")
                return model
    except Exception:
        pass
    print(f"  Router unavailable, using default: gemma:7b")
    return "gemma:7b"


def call_ollama(system: str, user: str, num_predict: int = 2400) -> str:
    """Call Ollama locally. Returns generated text."""
    payload = json.dumps({
        "model": MODEL,
        "system": system,
        "prompt": user,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate a Creative Bible PAL story (local Ollama, router-selected model)."
    )
    ...
    # 1b. Resolve model via router (falls back to gemma:7b)
    global MODEL
    MODEL = _resolve_creative_model(root)
