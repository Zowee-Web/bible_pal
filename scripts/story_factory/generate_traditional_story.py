...
# Canonical prompts shared with the Claude backend (story_prompts.py).
from story_prompts import (
    SYSTEM_PROMPTS_STORY_TRADITIONAL as SYSTEM_PROMPTS_STORY,
    KID_SYSTEM_PROMPTS_STORY_TRADITIONAL as KID_SYSTEM_PROMPTS_STORY,
    ...
    TRADITIONAL_RANGES as LOCKED_RANGES,
    KID_TRADITIONAL_RANGES as KID_LOCKED_RANGES,
    TRADITIONAL_SANITIZE_PROMPT,
    META_TEXT_REPAIR_INSTRUCTION,
)
from claude_validator import log_lane_warnings
...
def main() -> int:
    ...
    try:
        ...
        for length, (lo, hi) in ranges.items():
            ...
            # Gate 5 (WARN-only, adult only): lane identity validator.
            # Logs to assets/diagnostics/lane_validator_log.jsonl but does NOT
            # reject the story. Promotion to BLOCKING is a separate decision
            # after ~3-5 batches of calibration data.
            if not is_kid:
