"""Run the offline parent-control suite under a process-wide network kill
switch, with a mandatory positive control.

Deliberately a SIBLING of `run_probe_killswitch.py` rather than a
parameterisation of it: that runner is CI-enforced for Gate D and is left
byte-identical, so nothing about this harness can affect it.
...
— which matters here, because `OneShotTransport.send_once` deliberately
catches broad exceptions to classify transport failures.

Phase 1 (positive control): each patched mechanism is exercised directly.
If ANY completes, the switch is vacuous and the suite does NOT run.

Phase 2: only then is the suite imported and run, with the switch live.

    python3 scripts/tests/run_parent_control_killswitch.py
"""
...
def _positive_control() -> None:
    checks = (
        ...
        ("ssl.create_default_context", lambda: ssl.create_default_context()),
...
def main() -> None:
    ...
    unittest.main(
        module=None,
        argv=["run_parent_control_killswitch", "-v",
              "scripts.tests.test_probe_r2_parent_control"],
        exit=True)
