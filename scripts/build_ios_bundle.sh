...
# Persist picked sets for pubspec generation pass and smoke testing
...
staged_story_dirs=$(find "$STAGE_DIR/traditional" -maxdepth 1 -mindepth 1 -type d | wc -l | awk '{print $1}')
...
# directory-scoped, so audio absence in a text-only dir is what makes it
# text-only.
